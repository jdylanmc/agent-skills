...
1. If a new test file is added, register it explicitly in
...
2. Inspect tests for cancellation hazards. Do not define a test inside another
   test's synchronous callback; watch the `cancelled` count in `node --test`
   output, not only `fail`.
3. Keep assertions independent of line wrapping. Match whitespace-normalized
...
4. Run the validation sequence from the repository root:
...
5. After `derive-skill-graph.mjs --write`, inspect the diff. The deriver may
   update `used-by` on composed units and `allowed-tools` on molecules. It must
   not widen a routable skill's `allowed-tools`.
6. If the combined test run reports one intermittent failure, rerun once before
   assigning blame. Report both outputs honestly. Any persistent failure or any
   `cancelled` test is blocking.
7. Review the package against `agents/skill-coach.agent.md`. Fix every finding
...
8. Before shipping, verify no signature footer was added and no external skill
