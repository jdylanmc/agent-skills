...
export const DOWNSTREAM_AUTHORITY_WARNING = 'not-authority-until-separately-approved';
...
export function renderNfrProposal(proposal) {
  return [
    `# ${proposal.id}`,
    '',
    `- Revision: ${proposal.revision}`,
    `- Authority: ${proposal.authority}`,
    `- Approval state: ${proposal.approval?.state}`,
    `- Generated by decision: ${proposal.generatedByDecision}`,
    `- Justification: ${proposal.justification}`,
    `- Serves: ${proposal.serves}`,
    `- Threshold status: ${proposal.thresholdStatus}`,
    `- Threshold: ${proposal.thresholdStatus === 'threshold-unknown' ? 'threshold-unknown' : proposal.threshold}`,
    `- Scope: ${proposal.scope}`,
    `- Verification intent: ${proposal.verificationIntent}`,
    `- Source design: ${proposal.sourceDesign}`,
    `- Downstream authority warning: ${proposal.downstreamAuthorityWarning}`,
    '',
  ].join('\n');
}
...
export function validateNfrProposals(input = {}) {
  ...
  const proposals = input.proposals.map((proposal, index) => {
    ...
    const proposalPath = requiredText(proposal.path, `proposal ${id}.path`).replaceAll('\\', '/');
    if (path.isAbsolute(proposalPath)
        || proposalPath.split('/').includes('..')
        || !proposalPath.startsWith('docs/agent/nfr/')) {
      findings.push({ code: 'invalid-proposal-path', severity: 'high', subject: id });
    }
    if (proposal.downstreamAuthorityWarning !== DOWNSTREAM_AUTHORITY_WARNING) {
      findings.push({ code: 'missing-authority-warning', severity: 'high', subject: id });
    }
