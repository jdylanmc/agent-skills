...
1. `needs-discovery` when the confirmed source is materially incomplete or a
   product requirement needs evidence or scope Discovery did not settle.
2. `needs-decision` when a product decision, sibling conflict, or human nano
   approval remains unresolved.
3. `blocked` when source access, alignment, or freshness failed; persistence,
