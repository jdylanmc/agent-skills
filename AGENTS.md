...
- `includes` is the complete required-link mirror. `composes` is the exact direct subset of `includes` that names atoms or molecules. A routable `SKILL.md` also declares `composes`; a short, cohesive workflow may live directly in it without local atoms or molecules.
...
- Extract references for actual reuse, a meaningful enforcement boundary, or substantial independently useful detail. Do not split a short workflow merely to create atoms and molecules.
