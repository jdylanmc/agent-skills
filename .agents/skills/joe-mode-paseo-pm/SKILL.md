---
name: joe-mode-paseo-pm
description: "Human-enabled repository PM for Paseo. Choose a consented same-agent heartbeat or runtime-proven fresh schedule for one-minute passes; inspect, pause, resume or stop while preserving durable ownership and human merges."
...
4. Which host/repository worktree, accessible private evidence location and
...
Explain both one-minute modes, host availability and capability findings below
before asking for the run-mode choice. Capacity is delivery-owner lanes, not permission for
...
Once capabilities are known, ask **which mode the operator explicitly consents to**:

- **Same-agent heartbeat:** one dedicated or reused PM agent in the correct
  existing workspace receives `* * * * *` prompts. It returns/idles between
  bounded passes, retaining its conversation and wakeup duty. Recommend this
  compatible alternative on the known incompatible fresh scheduler.
...
Record the selected `wakeupMode` and explicit `wakeupConsent` decision reference,
including the explained conversation/lifetime difference. Previous interest in
...
a competing controller to work around that. Use `cron: "* * * * *"`, the approved
...
**Fresh:** use the current supported `create_schedule` schema, `cron: "* * * * *"`, explicit
...
not a pause/resume API; do not use it to bypass fixed one-minute cadence. If
