...
Preserve scoped operator choices and required IDs. Without preselection,
...
Separate work by outcome and ownership, not file count. Assignments
...
keep distinct deliveries or Shepherds moving rather than doing every job
in sequence. Small lookups need no agent.
...
unmerged is insufficient. One specification graph with one PR has one delivery
owner scheduling its workers. Do not also dispatch its child tickets
as competing root deliveries.

Squadron invocation does not authorize starting Ship,
...
Supply actual source evidence; do not assume workers inherited the conversation.
Include:
...
Writing workers need distinct isolated workspaces, not just disjoint files
in one checkout sharing a Git index. Use the existing delivery's
...
Every modifying worker uses [Changelog](../changelog/SKILL.md). Assign one integrated
changelog owner; other workers return categorized entry proposals, not competing
edits. Prefer terse exact messages without changing
the human's Caveman mode or dropping required packet fields.

## 3. Dispatch concurrently within real capacity

Use actual harness concurrent/background dispatch tools and current schemas.
Launch independent workers together where supported. Multiple calls
do not prove parallel execution; confirm returned IDs and runtime states.
Use configured model preferences and defaults, never invented model names.

Reserve bounded capacity for integration, independent review, human questions,
and existing monitors. Do not launch a second monitor for an owned PR
...
Continue independent work while agents run. Use completion notifications
or supported waits; do not repeatedly poll known workers.
Reuse existing agents for follow-up where supported. Queue human questions
through the parent; workers must not manufacture answers.

## 4. Reconcile and return ownership

A worker returns concrete results, changed artifacts/commits/PR IDs,
...
On failure, preserve partial work and the exact blocker; do not hide errors
behind an empty success-shaped report.

Verify the decisive result and integrate through the designated owner.
Inspect overlap and run the smallest combined checks covering interactions,
escalating only when impact or repository guidance requires it. Passing worker
tests do not prove the integrated result.
...
Clean up only confirmed finished, run-owned work after integration and
preservation checks. A return message does not make its worktree or monitor
