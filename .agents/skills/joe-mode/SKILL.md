...
The separate human-enabled [CMUX cockpit](../joe-mode-cmux/SKILL.md) and
[Paseo PM adapter](../joe-mode-paseo/SKILL.md) do not create another logical
...
CMUX's exact managed run and Paseo's saved activation, wakeup job and current
pass. Join an existing logical controller or arrange observed release and
acknowledged transfer; an idle tab or gap between scheduled passes is not
