...
For agent dispatch/return, load and execute [LIFECYCLE](../squadron/LIFECYCLE.md)
and give implementers [WORKER](../ship/WORKER.md). Preserve diagnosis-only scope;
even a read-only terminal return needs owner acceptance before agent retirement.
Repair delivery additionally executes DELIVERY's accepted custody and verified
provider non-draft gate, not a draft-publication stopping point.
